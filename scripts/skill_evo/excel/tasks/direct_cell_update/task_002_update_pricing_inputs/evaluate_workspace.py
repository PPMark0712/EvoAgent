...
INPUT_SAMPLES = [1, 2, 41, 88, 133, 200]
STATIC_CELLS = {"Pricing": ["A1", "B1", "C1", "D1", "E1"], "Notes": ["A1", "A2", "A3"]}
for idx in INPUT_SAMPLES:
    row_num = idx + 1
    STATIC_CELLS["Pricing"].extend([f"A{row_num}", f"B{row_num}", f"C{row_num}", f"D{row_num}", f"E{row_num}"])
...
def build_expected_formulas():
    ...
    discount_values = [row[2] for row in rows]
    renewal_values = [row[3] for row in rows]
    support_values = [row[4] for row in rows]
    return {
        "Pricing": {
            f"B{summary_row}": sum(base_values) / len(base_values),
            f"C{summary_row}": sum(discount_values) / len(discount_values),
            f"D{summary_row}": sum(renewal_values) / len(renewal_values),
            f"E{summary_row}": sum(support_values),
        }
    }
...
def main(args):
    target_file = os.path.join(args.workspace_path, "pricing_model.xlsx")
    gold_file = os.path.join(args.gold_path, "pricing_model.xlsx")
    evaluator = Evaluator("direct_cell_update/task_002_update_pricing_inputs", target_file, gold_file)
