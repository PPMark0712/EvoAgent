...
SAMPLE_ROWS = [2, 5, 34, 82, 126, 173, 201]
VALUE_CELLS = {"Expenses": ["A1", "B1", "C1", "D1", "E1"]}
STYLE_CELLS = {"Expenses": ["A1", "B1", "C1", "D1", "E1"]}
for row_num in SAMPLE_ROWS:
    VALUE_CELLS["Expenses"].extend([f"A{row_num}", f"B{row_num}", f"C{row_num}", f"D{row_num}", f"E{row_num}"])
    STYLE_CELLS["Expenses"].extend([f"B{row_num}", f"C{row_num}", f"D{row_num}", f"E{row_num}"])


def main(args):
    target_file = os.path.join(args.workspace_path, "expense_audit.xlsx")
    gold_file = os.path.join(args.gold_path, "expense_audit.xlsx")
    evaluator = Evaluator("cell_formatting/task_002_format_expense_audit_columns", target_file, gold_file)
    if evaluator.require_target() and evaluator.require_gold():
        evaluator.compare_sheetnames()
        evaluator.compare_sheet_dimension("Expenses")
