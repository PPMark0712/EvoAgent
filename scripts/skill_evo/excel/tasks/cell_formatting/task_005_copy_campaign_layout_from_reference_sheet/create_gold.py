...
def main(args):
    ...
    target = os.path.join(args.output_path, 'campaign_launch_queue.xlsx')
    if not os.path.isfile(target):
        build_workbook(target)
    wb = load_workbook(target)
    apply_styles(wb['Reference Layout'], GUIDE_ROW_COUNT)
    apply_styles(wb['Launch Queue'], DATA_ROW_COUNT)
    wb.save(target)
    wb.close()
