...
SEGMENT_FILL = PatternFill("solid", fgColor="FFF2CC")
...
def apply_styles(ws):
    ...
    for row_num in range(2, DATA_ROW_COUNT + 2):
        ...
        ws[f"E{row_num}"].fill = SEGMENT_FILL
...
def main(args):
    ...
    target = os.path.join(args.output_path, "revenue_tracker.xlsx")
    if not os.path.isfile(target):
        build_workbook(target)
    wb = load_workbook(target)
    apply_styles(wb["Revenue"])
    wb.save(target)
    wb.close()
