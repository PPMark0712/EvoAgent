...
from create_workspace import apply_full_styles, apply_local_emphasis, build_workbook, write_formulas


def main(args):
    os.makedirs(args.output_path, exist_ok=True)
    target = os.path.join(args.output_path, 'board_kpi_style.xlsx')
    if not os.path.isfile(target):
        build_workbook(target)
    wb = load_workbook(target)
    target_ws = wb['BoardKPI']
    for row_num in range(2, 180 + 2):
        write_formulas(target_ws, row_num, broken=False)
    apply_full_styles(target_ws, 180)
    apply_local_emphasis(target_ws, 180)
    wb.save(target)
    wb.close()
