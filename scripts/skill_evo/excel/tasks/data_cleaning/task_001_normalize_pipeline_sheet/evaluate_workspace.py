...
def main(args):
    target_file = os.path.join(args.workspace_path, "pipeline_clean.xlsx")
    gold_file = os.path.join(args.gold_path, "pipeline_clean.xlsx")
    evaluator = Evaluator(
        "data_cleaning/task_001_normalize_pipeline_sheet",
        target_file,
        gold_file,
    )
