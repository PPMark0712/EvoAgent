...
def main(args):
    ...
    target = os.path.join(args.output_path, "pipeline_clean.xlsx")
    wb = Workbook()
    ws = wb.active
    ws.title = "Clean"
    ws.append(["Deal ID", "Stage", "Amount", "Owner", "Region"])
    raw_rows = messy_rows()
    for index, row in enumerate(canonical_rows()):
        ws.append([row["deal_id"], row["stage"], row["amount"], raw_rows[index][3], row["region"]])
    for cell in ws[1]:
